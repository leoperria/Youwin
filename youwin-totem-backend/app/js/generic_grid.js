...
Ext.extend(Application.apiGrid.WinList, Ext.util.Observable, {
  ...
  buildMenu:function(){
    ...
    if(typeof this.config.gridButtons!='undefined'){
      menuItems.push(this.config.gridButtons);
    }
    return menuItems;
  },
  ...
  buildGridPanel: function(){
    ...
    if (this.config.paged){
      gridPanelConfig.bbar=this.bbar;
    }

    this.gridPanel=new Ext.grid.GridPanel(
      Ext.apply(gridPanelConfig,this.config.gridConfig)
      );
  ...
  refreshPanel:function(){
    if (this.config.paged){
      if (this.gridPanel.getStore().getCount()==0){
        this.gridPanel.getStore().reload({params:{start:0,limit:this.bbar.pageSize}});
      }else{
